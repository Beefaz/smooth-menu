<template>
  ...
  <PlanetModel
      ...
      :model="selectedPlanet.planetModel"
      :ios-model="selectedPlanet.iosModel"
  />
  <SmoothMenu
      v-if="currentMenuParams"
      class="navigtion"
      :item-size="currentMenuParams.itemSize"
      :links="links"
...
<script>
import SmoothMenu from './components/SmoothMenu.vue'
...
export default {
  ...
  methods: {
    checkScreenSize(e) {
      const screenWidth = e.currentTarget.window.innerWidth;

      if (screenWidth > 0 && screenWidth < 800) {
        return this.currentMenuParams = this.menuBreakPoints.sm;
      }

      if (screenWidth > 800 && screenWidth < 1200) {
        return this.currentMenuParams = this.menuBreakPoints.md;
      }

      return this.currentMenuParams = this.menuBreakPoints.xl;
    },
    changeModel(id) {
      this.selectedPlanet = this.links.find((link) => link.id === id);
    },
  },
}
</script>
